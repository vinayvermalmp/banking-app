...
public class TraceFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (isCorrelationIdPresent(reqHeaders)) {
            logger.debug("inwove-correlation-id found in tracing filter {}. ",
                    filterUtility.getCorrelationId(reqHeaders));
        }else {
            String correlationID = generateCorrelationId();
            exchange = filterUtility.setCorrelationId(exchange , correlationID);
            logger.debug("inwove-correlation-id generated in tracing filter: {}.", correlationID);
        }
